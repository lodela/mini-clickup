...
const formSchema = z.object({
  name: z.string().min(1, "epicNameRequired"),
...
export default function CreateEpicModal({
  ...
  const { create } = useEpics(projectId);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      toast.success(t('epics.createSuccess'));
      onSuccess();
      onClose();
      form.reset();
    } catch (err: any) {
      toast.error(err?.message || t('epics.createError'));
    } finally {
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <DialogTitle className="text-xl font-bold tracking-tight text-white/90">
                {t('epics.createNewEpic')}
              </DialogTitle>
              <DialogDescription className="text-white/50 text-sm">
                {t('epics.createDescription')}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
